...
function SignIn() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setLoading(true);
      const response = await connect.post(Apis.SIGN_IN, {
        email: email.trim(),
        password,
      });
  ...
  const signupWithGoogle = async () => {
    try {
      await signInWithPopup(auths, googleProvider);
      ...
      <div className="signin-wrapper">
        ...
        <div className="signin-right">
          ...
          <p className="signin-subtitle">
            Hello, friend! I'M Your Sky Card. Let's get in touch!
          </p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="signin-google text-4xl">
              <Button onClick={signupWithGoogle} className="google-button">
                ...
                />
                <span className="text-3xl"> Sign in with Google</span>
              </Button>
            </div>
          </form>
